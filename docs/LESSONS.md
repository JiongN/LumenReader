...
- **元凶与修法**：定位到拖动时**每个指针事件 → 一次整页重光栅化**（`PDFView.draw=1.0/步`）。
  ...
  修法：把宽度写入**合并到每个显示刷新应用一次**（`LivePanelWidth` + `CADisplayLink`），
  屏幕本来只按刷新率显示，用户看不出差别，重活从「按事件率」降到「按刷新率」。
- **实测（本机，每帧模拟 3 次指针写入）**：
  - 合并前：`ReaderContainerView.body=3.02/步  PDFView.layout=2.80/步  PDFView.draw=2.80/步`，
    主线程停顿 p95=20.7ms、`>16.7ms` 占比 10%。
  - 合并后：`~1.1/步`，p95≈10ms、占比 2%。
  - **证伪**（同一构建，`--jank-no-coalesce 1` 关掉合并）：回到 `3.02/步`——证明是合并本身在起作用。
  - 进程 CPU（另一把尺子）：拖动 2727ms（45ms/步，主线程）、滚动 752ms（12.5ms/步，后台线程）。
...
- **沉淀**：`Sources/LumenApp/Shell/JankAudit.swift`（通道 + watch）、`Sources/LumenApp/Design/LivePanelWidth.swift`
  （修法）；读法与两通道分工见 `docs/VERIFY.md` 第四节「验证连续交互的卡顿」。
