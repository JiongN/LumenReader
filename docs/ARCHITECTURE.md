...
### 3.7 面板宽度：拖动期间用本地状态，松手才提交

```
拖动中   PanelResizeHandle → 容器 @State liveWidth → .frame(width:)   ← 每帧，不写设置
松手     onCommit(live) → SettingsStore.commitSidebarWidth(_:maxWidth:)
                          → UISettings.sidebarWidth 的 didSet（第二道钳制闸）
                          → @Published → 布局刷新
```

宽度只有**一个真相源**（设置），但拖动期间不再每帧写它。原因：`AppSettings`
是 `@Published` 的整个结构体，每帧写一次会让所有观察 `SettingsStore` 的视图
整棵失效——阅读区（PDFKit / WKWebView）与两块 `.regularMaterial` 背景都在其中，
表现就是分隔线拖起来抖。改成「本地状态驱动布局 + 松手提交一次」之后，
每帧只影响一条 `.frame(width:)`。

`liveWidth` 只在手势期间非 nil，手势结束立刻置回 nil，因此**不是**第二份真相源：
设置页改宽度、双击复位、自检写入这些外部改动照旧实时生效（这条链路由
`--resize-report` 盯着）。

**上限按窗口宽度动态收窄**（`PanelWidthPolicy`）：窗口只有 920pt 时，
两侧面板都拉到静态上限会把正文整个挤没，所以上限 = 窗口宽 − 图标栏 − 其它面板
...
预算的分配顺序：`预算 = 容器宽 − 图标栏 − 分隔线`；`可用 = 预算 − 阅读区保底`。

1. 两侧需求装得进「可用」→ 各自按需求给；
2. 装不进 → **被钉住的一侧足额拿**，对侧吸收差额（但不低于自己的下限）；
   不拖动时钉住侧栏，拖动时钉住被拖的那一侧（分隔线要跟得住鼠标）；
3. 连两侧下限都装不下（容器 < 534pt）→ 下限优先，阅读区让位（**降级**，
   文档里写明）；
4. 最后一道闸：两侧之和不得超过预算。宁可面板比下限还窄，也不让图标栏
   被顶出屏幕——面板窄是难用，入口消失是不能用。

**不拖动时为什么钉住侧栏而不是两侧等比压缩**：等比看着更公平，但会让
「写入 X → 渲染 X」这条关系在窄窗口下失效——写 266 只渲染出 225，
于是拖到底再松手、宽度反而比拖动中更窄。钉住一侧则天然是**不动点**：
把结果再喂回这个算式得到的是同一组值，这正是「拖完不弹回」的数学保证。
让侧栏优先的理由：侧栏装的是目录 / 搜索结果 / 批注这类结构化列表，
窄到 200pt 以下就开始横向裁字且没法靠重排补救；AI 面板装的是会自己换行的
正文与气泡，窄 100pt 只是行长变短，而且它还有「整个收起」这条退路。

**松手时对侧也一起落库**：拖完只存被拖的那一侧的话，下一次布局会按两个偏好
重新分配，对侧立刻弹回原宽度——表现为「拖完弹回去」。存下的就是用户此刻看到的版式。
