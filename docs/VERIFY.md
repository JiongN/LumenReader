...
> **当前验收规则（2026-09-23）**：本节 `--jank-report` 是合成事件和程序化拖动诊断，不能验收真实触控板滚动。先核对运行进程与构建标识，避免把旧 `dist/.trash` 应用当成当前版本；再用同一份代表性大 PDF、同一窗口条件，让用户实际连续滚动并松手滑行，同时观察空白、缩放和面板切换。可用下文 `--jank-watch` 被动辅助定位，最终以真实操作的前后体验为准。计时器迟到不是实际屏幕帧率，采样栈的命中次数不是同步调用次数。
...
| `--jank-report 1` | 合成连续动作的重活计数与主线程计时迟到 | 「程序化路径有没有重复计算」 | 每步重排 / 重光栅化；不代表真实触控板帧率 |

用户说「拖动时抖 / 滚动卡」时，单次性能报告可能全绿；先用真实操作确认问题，再用 `--jank-watch` 对齐手势时段，`--jank-report` 只用于辅助检查可程序化的路径。
...
2. **主线程计时迟到**只用于同条件对照。`>16.7ms 的采样占比` 下降是辅助证据，不能直接推断触控板手感改善。
...
> 历史读数曾是活跃段 CPU ≈3 个核、本层计数器接近 0。当时据此推断卡顿在 PDFKit 渲染管线；后续隔离页码/视口状态发布解决了快速滚动空白，证明该推断不足以排除应用侧更新。见 [后续复核](PDF-PANELS-20260923.md)；三旋钮实验仅说明那些开关在当时配置下无收益。
...
> 2026-09-23 更新：本节“瓶颈是整页光栅化”的旧归因不充分。后来分别发现自动 Vision 分析负载、滚动期页码/视口发布与面板动画布局问题；见 [滚动记录](PDF-SCROLL-20260922.md)及[面板与状态隔离复核](PDF-PANELS-20260923.md)。历史测量保留，不能作为当前流畅度验收结论。

当时曾依据用户实机 `--jank-watch`（2 秒一窗）将滚动卡顿归因于 **PDFKit 渲染管线**：活跃段进程 CPU 在 2 秒里烧掉
...
> **这组实验的有效结论**：三个旋钮都未明显省下 CPU，渲染默认**维持原样**；不能据此断定整项卡顿的唯一瓶颈。
