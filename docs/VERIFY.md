...
| `--jank-watch 1` | **被动监视**（与 `--jank-report` 相反，不驱动任何东西）：正常启动运行，后台每 2 秒把「主线程停顿 p50/p95/max、>16.7ms 次数、各计数器增量、进程 CPU 增量、当前页码/滚动偏移」写一行到 `/tmp/lumen-jank-watch.log`，**交给用户用真触控板产生手势**复现（合成事件搓不出连续惯性滚动）。纯读不写设置（`suppressSave`），日志只在 2 秒汇总时落盘，不在绘制路径里写文件 |
...
1. **每步重活**是定位项。拖动段 `PDFView.draw(重绘)=1.0/步` 说明**每个拖动步长都在整页重光栅化**——
   ...
   按显示刷新合并宽度写入后降到 `~1.1/步`。
2. **主线程停顿**是打分项。`>16.7ms 的采样占比` 从 10% 降到 2% 就是可感的手感改善。
...
4. **驱动自证**必须先绿再读数：滚动段会打印「页码 a → b，滚动偏移 x → y ✅ 确实滚了」；
   拖动段打印「宽度… → a…b，每帧 N 次指针写入」。任何一段没动，那一整段的读数是死的，别引用。
...
```
+12.0s 停顿 p50=0.31 p95=8.42 max=58.10 >16.7ms=6/121(5%) | CPU +1840ms | 页=7 偏移=98421 | body=3 ai=1 side=1 thumbBody=2 update=0 layout=0 draw=0 thumbR=1 pos=1
```
